...
<div class="container">
    ...
    <form method="POST" action="{{ route('encashment_condition.update', $leaveEncashmentApprovalCondition->id) }}">
        @csrf
        @method('patch') 
        <div class="form-group">
            <label>Approval Type:</label>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="approval_type" id="hierarchy" value="Hierarchy" {{ $leaveEncashmentApprovalCondition->approval_type === 'Hierarchy' ? 'checked' : '' }}>
            ...
            <div class="form-group" id="hierarchyFields">
                ...
                <select name="hierarchy_id" id="hierarchy_id" class="form-control" {{ $leaveEncashmentApprovalCondition->approval_type !== 'Hierarchy' ? 'disabled' : '' }}>
                    <option disabled selected>Select Hierarchy</option>
                    @foreach($hierarchies as $hierarchy)
                        <option value="{{ $hierarchy->id }}" {{ $leaveEncashmentApprovalCondition->hierarchy_id == $hierarchy->id ? 'selected' : '' }}>{{ $hierarchy->name }}</option>
                    @endforeach
                </select>
                @error('hierarchy_id')
            ...
            <div class="form-group" id="maxLevelFields">
                ...
                <select name="MaxLevel" id="MaxLevel" class="form-control" {{ $leaveEncashmentApprovalCondition->approval_type !== 'Hierarchy' ? 'disabled' : '' }}>
                    <!-- Options will be populated dynamically based on the selected hierarchy -->
                </select>
            </div>

            <div class="form-check">
                <input class="form-check-input" type="radio" name="approval_type" id="single_user" value="Single User" {{ $leaveEncashmentApprovalCondition->approval_type === 'Single User' ? 'checked' : '' }}>
            ...
            <div class="form-group" id="employeeFields">
                ...
                <select name="employee_id" id="employee_id" class="form-control" {{ $leaveEncashmentApprovalCondition->approval_type !== 'Single User' ? 'disabled' : '' }}>
                    <option disabled selected>Select Employee</option>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ $leaveEncashmentApprovalCondition->employee_id == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->employee_id }})</option>
                    @endforeach
                </select>
            </div>

            <div class="form-check">
                <input class="form-check-input" type="radio" name="approval_type" id="auto_approval" value="Auto Approval" {{ $leaveEncashmentApprovalCondition->approval_type === 'Auto Approval' ? 'checked' : '' }}>
